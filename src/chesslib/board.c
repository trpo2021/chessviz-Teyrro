...
bool border_check(struct chess_info chess){
	...
	&& ((chess.y_end >= 0 && chess.y_end <= 7) && (chess.x_end >= 0 && chess.x_end <= 7))){ 	
		return true;
	}
	return false;
}
...
bool type_check(struct game game,struct chess_info *chess){ 
	...
	if ((*chess).move_type == '-' && game.table[(*chess).y_end][(*chess).x_end] == ' ' && game.table[(*chess).y][(*chess).x] != ' '){
		
		return true;
	}
	if ((*chess).move_type == 'x' && game.table[(*chess).y_end][(*chess).x_end] != ' ' && game.table[(*chess).y][(*chess).x] != ' '){
